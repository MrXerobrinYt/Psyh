(function() {
    ...
    if (!visitorId) {
        // Генерируем уникальный ID с высокой энтропией
        visitorId = 'user_' + Date.now() + '_' + Math.random().toString(36).substring(2, 15) + '_' + performance.now();
    ...
    function getActiveVisitors() {
        ...
        try {
            ...
            const now = Date.now();
            // Оставляем записи не старше 60 секунд (для надёжности)
            Object.keys(parsed).forEach(id => {
                if (now - parsed[id].timestamp > 60000) delete parsed[id];
            });
            return parsed;
        } catch(e) { return {}; }
    }
